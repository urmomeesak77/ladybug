...
- [X] T056 [P] Playwright e2e in `frontend/e2e/moderation.spec.ts` (Playwright `testDir` is `e2e/`, not `tests/e2e/`) — role gate (member: no nav link + redirected home; admin/superuser: link + console), browse the seeded 20-post corpus with the numbered page link marked current, and a Deactivate→Activate in-place round-trip. Admin provisioned via `frontend/e2e/helpers/adminSetup.ts` (`user:make-superuser` inside the `backend-e2e` container). **Verified: 2 passed against the isolated `ladybug-e2e` stack** (`scripts\e2e.ps1 e2e/moderation.spec.ts`).
- [X] T057 Verify theming/a11y/responsive on `ModerationPage`/`ModerationTable` — confirmed: state is text+aria-hidden glyph, never colour alone (FR-014); the danger confirm uses the theme `--color-error` token AND the word "delete"; `<caption>` + `th[scope=col]`, `<img alt>`/placeholder `role=img aria-label`, labelled `<button>` action controls and `aria-current` page links; `.moderation-table__scroll { overflow-x: auto }` + `white-space: nowrap` keeps the wide table scrolling inside its box so the page never scrolls horizontally (SC-008, Principles IV/VIII); all colours are shared `--color-*` tokens with dark-mode overrides (`prefers-color-scheme`).
- [X] T058 Real gates via Docker held ≥90% both stacks (verifier this session): backend Pint clean + PHPUnit **243 passed / 99.08%**; frontend ESLint clean + tsc clean + Vitest **429 passed / 97.93% lines** (moderation modules 100%). Phase 7 adds only e2e specs (excluded from unit coverage, covered by ESLint).
- [X] T059 Quickstart scenarios covered by the automated suites: US2 access + US1 browse + US3 toggle by the e2e spec (T056); FR-012 uploader fallback, FR-011 thumbnail placeholder, FR-016 delete confirm, and US4 delete/restore + public-view hiding by the unit/feature suites. Not machine-exercised (operator smoke only): live image/YouTube thumbnail *rendering* (SC-003/SC-004) — the e2e stack is media-less by design — and the manual light/dark theme toggle.
