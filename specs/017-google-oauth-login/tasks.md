...
- [X] T052 [P] [US6] Extend `backend/tests/Unit/Http/Resources/UserResourceTest.php` with `has_password` (`password !== null`) and `google_linked_at` (the link's `created_at`, else `null`), and assert `provider_user_id` appears nowhere in the payload (FR-022, SC-009). Four cases added: password-only, Google-only, a linked password account carrying **both** doors, and the FR-022 guard (which checks the encoded payload for the subject **value**, not just the key name, so a future field carrying it under another name is caught too). **The one edited assertion in the whole feature**: `test_it_exposes_only_the_safe_public_fields_in_order` asserts an **exhaustive** key list, so adding two keys necessarily moves it. That is a payload-**shape** assertion, not a behavioural one — SC-007's regression gate is about what the password door *does* (`contracts/password-login-invariant.md` §3), and the list is deliberately exhaustive precisely so a field cannot join the payload without someone deciding it is safe to disclose. Recorded here so T065's `git diff` finds exactly one such edit and knows why
- [X] T053 [US6] Add `has_password` and `google_linked_at` to `backend/app/Http/Resources/UserResource.php` (data-model.md §6), with a `why` comment noting the resource is only ever returned for the requester's own account. `google_linked_at` is the **link's** `created_at`, not the account's — on a US3 auto-link the two differ, and the later date is the one that answers "since when could this be signed into with Google?"
- [X] T054 [P] [US6] Extend `frontend/tests/lib/authApi.test.ts` and `frontend/tests/lib/authModel.test.ts` with the mirrored `hasPassword` / `googleLinkedAt` fields and all four `AuthModel.signInMethod()` outcomes from `contracts/ui-surface.md` §3, including the unreachable total fallback. **Note**: unlike the backend's exhaustive key list, feature 007's `mapUser` `toEqual` assertion did **not** have to move. The new fields are mapped by straight pass-through (the idiom the rest of the mapper already uses), so against the 007 fixture — written before these fields existed — they map to `undefined`, and `toEqual` ignores undefined properties. The new cases therefore carry their own payloads rather than extending the shared fixture, which would have moved that assertion for no reason
- [X] T055 [US6] Add `hasPassword: boolean` and `googleLinkedAt: string | null` to the `AuthUser` type and its mapping in `frontend/src/lib/authApi.ts`, and `signInMethod(user)` to `frontend/src/lib/authModel.ts`. The link check is written as a truthiness test rather than `=== null`, so a legacy or malformed payload degrades to "Email and password" instead of claiming a Google door that is not there
- [X] T056 [US6] Extend `frontend/tests/pages/AccountPage.test.tsx` with all four sign-in-method strings, then append the `<dt>Sign-in method</dt><dd>{AuthModel.signInMethod(user)}</dd>` row to the existing `<dl class="account__details">` in `frontend/src/pages/AccountPage.tsx` — words, never an icon or colour (Principle IV). The tests read the value as the `<dt>`'s `nextElementSibling`, which asserts the row is a real term/definition **pair** inside the list rather than two loose lines that merely look like one; a fifth case asserts the `<dd>` has **no element children at all**, which is Principle IV stated as something a test can fail — a badge or a coloured dot would break it

### The register page and `?error=` rendering (FR-001, FR-007, FR-026)

- [X] T057 [P] [US6] Extend `frontend/tests/pages/RegisterPage.test.tsx`: the Google button is present, the visible `or` label is in the DOM as text, and each `?error=` code renders its sentence into a `role="alert"` element (with `?error=<script>` and an unknown code both rendering the generic `provider` sentence and no markup). **Note**: unlike `LoginPage`, `RegisterPage` today has **no** form-level error region — only per-field `AuthField` errors — so T058 adds one; this task asserts the added element, not a pre-existing one. All seven codes are driven through one `it.each` table, plus an unknown code, the `<script>` payload, DOM order, and the case that must stay silent: an ordinary `/register` visit raises **no** alert at all. The existing `renderRegister` helper gained an optional second argument (defaulting to `/register`), the same shape `renderLogin` already had — no existing call site or assertion changed
- [X] T058 [US6] Add to `frontend/src/pages/RegisterPage.tsx`: the `.auth-alt` block with the visible `or` label; **a new form-level `<p className="auth-form__error" role="alert">` region** matching `LoginPage.tsx`'s conditional one byte for byte (`RegisterPage` has none today — `LoginPage` and `UploadPage` are the only two files carrying that element); and `?error=` rendering via `useSearchParams()` through `GoogleAuth.errorMessage()` into it. The register form's existing field-level behaviour is unchanged — this adds a sibling region, it does not touch `RegisterFields` or validation. **Deviation from the task's letter, deliberately**: the button is rendered as `<GoogleSignInButton />` with **no** `redirectTo`. `RequireAuth` is the only thing in the SPA that plants `location.state.from`, and it plants it on `/login` alone — nothing can arrive at `/register` carrying a blocked destination, so a `redirectTo` here would be plumbing that no honest test could exercise and that no visitor could reach. A sign-up through Google has nowhere to return to but the feed, which is what the callback's server-side default already does
- [X] T059 [US6] Extend `frontend/tests/pages/LoginPage.test.tsx` with the same `?error=` cases, asserting the `disabled` code renders the **identical sentence** `LoginPage` already shows for a disabled password login (SC-006), then add the `?error=` rendering to `frontend/src/pages/LoginPage.tsx`. The SC-006 case does not restate the sentence as a literal: it drives a real disabled-account login, **captures what the page rendered**, then re-renders at `/login?error=disabled` and asserts the two are the same string — so a divergence in *either* sentence fails it, which a repeated literal could not catch. One case beyond the task: with both sources present the page's **own** last failure wins, because a submit the visitor just watched fail describes a newer event than the `?error=` code still sitting in the URL

### Theme, layout and e2e (FR-028, SC-010, Principle VIII)

- [X] T060 [US6] Add `.auth-alt`, `.auth-alt__label` and `.google-button` to `frontend/src/styles/theme.css` using the existing theme custom properties — both light and dark defined, nothing hard-coded, relative units, ≥44 × 44 CSS px touch target, full width at narrow widths. The Google mark's SVG keeps its official brand colours in both appearances, by design (`contracts/ui-surface.md` §4). Dark mode needs no rule of its own: the file overrides **tokens only** inside `prefers-color-scheme`, so styling in tokens is what makes both schemes true. The hairlines beside the word are `::before`/`::after` **on the label**, so deleting them changes nothing about what the row means — which is the test of whether a signal is carried by text (FR-026). The button is given the submit button's exact shape rather than a filled accent: the two sign-in methods are equals
- [X] T061 [US6] Create `frontend/tests/e2e/google-signin.spec.ts` — presence, accessible name, keyboard reach and activation, 320 px and desktop, light and dark, on **both** `/login` and `/register`. Plus the one FR-030 assertion that does not need a provider: load `/login?error=cancelled`, confirm the message renders, **reload**, and confirm the same page and message with no request to `/api/auth/google/*` (the `?error=` code is a display input, so a refresh re-runs nothing). Otherwise presence only: the round trip is deliberately not e2e-tested (research D16), which is possible because the button renders unconditionally (research D12). **Three corrections found during implementation, all mine, all found by running it** (7 tests, green on the isolated stack, and the full 39-spec suite green with them): (1) tabbing straight after `goto()` reached only the site logo and the left menu and then wrapped — because `RequireAnon` renders **nothing** until the session probe resolves, so `<main>` was still empty. The helper now waits for the control first, and keeps a **focus trail** in its failure message, which is what diagnosed this in one run and would name a focus trap the same way. (2) "full width" was first asserted against the viewport, which is wrong at desktop: the `.auth` column is capped at 600 px, so the control is 590 px on a 1440 px screen. It is now measured against the **password form's own submit button** — the same width at every viewport, which is the real requirement and states the equality directly. (3) Keyboard activation turned out to be drivable end to end after all: with the e2e stack's empty `GOOGLE_*` values, Enter on the button performs the whole navigation and comes back to `/login?error=provider` with the retryable sentence — the unconfigured-deployment answer of research D12, proved in a browser rather than argued. The theme case compares **computed** styles between the two schemes (background and text must both move) and pins the mark's `fill` to `rgb(66, 133, 244)` in both, which is §4's brand-colour exception stated as an assertion

**Checkpoint**: all six user stories complete. Both suites and both linters green.

**T061a — added after the quality gate, before the phase commit.** The `commit-quality-verifier`
returned PASS but flagged the 50-line function rule (`docs/CODING_CONVENTIONS.md`, Principle II)
on both auth pages, and correctly separated the two cases: `LoginPage` was **already** over budget
at `HEAD` (62 lines), while `RegisterPage` was **compliant at 46** and this phase's `?error=`
handling plus the `.auth-alt` block pushed it to 57 — a violation this diff *introduced*. Fixed by
extracting the two pieces both pages now share, which is also the reuse the duplication was asking
for: `frontend/src/hooks/useGoogleRefusal.ts` (the page's own failure, else the `?error=` sentence —
one reading of the parameter, so the same code cannot grow two different sentences on two pages,
which is what SC-006 forbids across the two front doors) and `frontend/src/components/AuthAlt.tsx`
(the `or` label + the button). Both carry mirrored tests (quickstart §2.1): `tests/hooks/
useGoogleRefusal.test.tsx` and `tests/components/AuthAlt.test.tsx`, 8 cases, both files at 100%.
`RegisterPage` is back **under** the budget at 48; `LoginPage` is 61, so this phase now leaves it
**smaller** than it found it. Getting it under 50 means extracting feature 007's `handleSubmit`,
which is not this phase's change to make. The rendered DOM is byte-identical, which is why the
extraction is provable: every page test and all 12 re-run e2e cases stayed green without an edit.

**Phase 8 gates, run and recorded** (after T061a): backend `vendor/bin/pint --test` PASS (168
files), `php artisan test` **931 passed**, clover **98.91%** against the 90% floor; frontend
`npm run lint` clean, `npx tsc -b` clean, `npm run test` **852 passed (90 files)** with coverage
**98.76% lines / 98.67% statements**; `scripts\e2e.ps1` **39 passed** including the 7 new ones,
and the auth + google-signin specs (12) re-run green after the extraction. Production files
touched: `UserResource.php`, `authApi.ts`, `authModel.ts`, `AccountPage.tsx`, `RegisterPage.tsx`,
`LoginPage.tsx`, `theme.css`, plus the two new files above — no new dependency, no new route.
...
- [X] T062 [P] Document the Google client setup in `docs/DEPLOYMENT.md`: creating the OAuth 2.0 Web-application client, the two authorised redirect URIs (they must **byte-match** `GOOGLE_REDIRECT_URI`), the `openid email profile` consent scopes, the three server env keys, and the migration-rollback caveat from research D6 (restoring `NOT NULL` once passwordless accounts exist is a procedure, not a command — quickstart §6). Landed as a new **"The Google OAuth client (017)"** subsection under §3 (first-time server setup), because that is where the other two "fill this in by hand after `setup.sh`" secrets already live — the section's list of manual steps went from **two to three**, since a client id and secret that `setup.sh` cannot generate are exactly that kind of step. Written past the task's letter in two places, both because the failure is otherwise undiagnosable from inside the app: (1) `redirect_uri_mismatch` is called out as a **Google-served** screen that runs no application code, so none of the feature's own seven error codes can appear for it — the symptom to recognise is a Google-branded error page rather than a Ladybug one; (2) leaving the keys empty is documented as a **supported** state (button renders, flow refuses with `?error=provider`), because that is what the e2e stack runs on and an operator finding that message should look at env plumbing, not at a bug. **Also added a post-deploy verification bullet** to §7's checklist naming both of those symptoms and which one points at env vs. at the console — the checklist already covers every other user-facing door and the Google door was the only one missing
- [X] T063 [P] Run `grep -r provider_user_id` across `backend/app`, `backend/routes`, `frontend/src` and confirm it appears in no resource, no route, no log line and no frontend file (FR-022, SC-009). **PASS**, verified by running the three greps scoped exactly as the task names them (a repo-wide grep would have been drowned by the spec artifacts and proved nothing). `backend/routes` and `frontend/src`: **zero hits**. `backend/app`: **six**, and every one is internal — two `why` comments (`UserResource.php:20`, `IdentityLinkService.php:185`), the `$hidden` declaration (`UserIdentity.php:40`), one explanatory comment (`UserIdentity.php:24`), and the service's own query column and assignment (`IdentityLinkService.php:81, 192`). No serialization, no route parameter, no log line. The `UserResource.php` hit is worth naming precisely because it is the one file that *would* leak it: the mention there is the comment explaining why the field is absent
- [X] T064 Run the real CI gates and record the output: `docker compose exec backend vendor/bin/pint --test`; `docker compose exec backend php artisan test --coverage-clover=coverage.xml` then `python .github/scripts/check_coverage.py backend/coverage.xml`; `cd frontend && npm run lint && npm run test -- --coverage`. Both stacks ≥90% (SC-011). The clover file is written to `/app/coverage.xml` **inside** the container and read from `backend/coverage.xml` on the host — the same file, because `backend/` is bind-mounted; without that mount the second command needs an explicit `docker compose cp`. **All four gates run and green:**

  | Gate | Result |
  | --- | --- |
  | `vendor/bin/pint --test` | **PASS**, 168 files, PSR-12 |
  | `php artisan test --coverage-clover` | **931 passed, 2513 assertions**, 365.84s, exit 0 |
  | `check_coverage.py backend/coverage.xml 90` | **98.91%** (min 90.00%) — *coverage gate passed* |
  | `npm run lint` | clean, exit 0, no output |
  | `npm run test -- --coverage` | **852 passed (90 files)**, 98.83% lines / 98.74% statements |

  **Two corrections to how the gate was *invoked*, both mine and both worth recording because either one could have produced a false PASS.** (1) `check_coverage.py` takes the threshold as a **required second argument** — called with the path alone it exits `2` on a usage error, which a `&&` chain or a glancing read of "no failure output" would have taken for success. It is now invoked exactly as `.github/workflows/ci.yml:72` invokes it, threshold `90` spelled out. (2) The clover file is **not** proof of the run that just finished merely by existing — an earlier `/app/coverage.xml` from a previous session was sitting there with a nearly identical byte count, and container-vs-host timestamps were not comparable at a glance. Freshness was therefore established from the file's own `generated=` epoch (24 s old at the moment the gate ran), not from `ls`. **Per-class coverage for every file this feature added, read out of the same clover run**: `GoogleAuthController` 39/39 stmts, 9/9 methods · `GoogleOAuthService` 44/44, 5/5 · `IdentityLinkService` 40/40, 7/7 · `OAuthFlowState` 22/22, 5/5 · `GoogleIdentity` 19/19, 4/4 · `Jwt` 20/20, 4/4 · `UserIdentity` 5/5, 2/2 · `OAuthFailure` 1/1, 1/1 — **100% statements and 100% methods across all eight**, plus `UserResource` (touched by T053) at 11/11, 1/1. The frontend line figure moved by 0.07 pp between two consecutive runs of an unchanged tree (98.76 → 98.83), from a nondeterministic branch in `useScrollRestoration` — noted so the difference is not later mistaken for a regression; both readings clear the 90% floor by more than eight points
- [X] T065 Confirm the features 007–015 suites pass **unmodified** — `git diff` shows no edited assertion in any pre-existing test file (SC-007, `contracts/password-login-invariant.md` §3). **PASS.** Measured against `git merge-base master HEAD` (`235b1fd`), not against the previous commit, so the whole feature is in view. Pre-existing test files: **10 modified, +599 / −6**. Because additions cannot break a prior assertion, the entire question is those **six deleted lines**, and every one was read: (a) `SchemaTest.php` — the `insertUser` helper's signature and doc comment (`void`→`int`, `insert`→`insertGetId`) so callers can hang a `user_identities` row off the new id; (b) `RegisterPage.test.tsx` — the `renderRegister` helper's signature and its hard-coded `initialEntries`, gaining the optional route argument `renderLogin` already had; (c) `UserResourceTest.php:1501` — the exhaustive key list, **the one edited assertion in the feature**, exactly as T052 predicted and for the reason recorded there (an exhaustive-shape assertion must move when a field is deliberately added; SC-007 governs what the password door *does*). So: two helper-plumbing changes and one documented shape assertion. **Zero** behavioural assertions edited. Pre-existing **production** files were audited the same way (11 modified, +156 / −1): the single deletion is `LoginPage.tsx`'s `formError` → `refusal` swap from T061a — same element, same `className`, same `role="alert"`, only the variable renamed. Green-ness is not inferred from the diff: it is T064's 931-test run, which includes every 007–015 suite
- [X] T066 Run `.\scripts\e2e.ps1` and confirm the Playwright suite, including `google-signin.spec.ts`, is green against the isolated stack with empty `GOOGLE_*` values. **This feature's spec is green; the suite as a whole is not, and the one failure is pre-existing and unrelated.** `google-signin.spec.ts` is **7/7** in both clean full runs — presence, accessible name, keyboard reach and activation, 320 px and desktop, light and dark, on both `/login` and `/register`, plus the FR-030 reload case — all against the stack's **empty** `GOOGLE_*` values, which is the point of the task. Suite totals, twice, identically: **38 passed, 1 skipped, 1 failed** of 40. The failure is `upload.spec.ts:66` (`expect(img.meme-media__image).toHaveCount(0)` → received 1), and it is **not 017's doing**, established from commit ancestry rather than from inference: the behaviour it trips on — a pending meme being visible to *its own uploader*, with a `HiddenNotice` banner — comes from `4c44336` (backend viewer-aware visibility), `874a9ce` (the banner) and `36e2a10`, and `git merge-base --is-ancestor` confirms **all three are on `master` and predate this branch's merge-base** (`235b1fd`). The spec's own comment still describes the superseded behaviour ("renders 'not found' to the very person who just uploaded") and calls the confirmation "unspecced follow-up work" — that follow-up shipped and the assertion was never updated. Playwright's `error-context.md` snapshot shows the page rendering the pending banner *and* the image to the signed-in owner, which is the shipped behaviour, so the test is stale rather than the app broken. It reproduces in full-suite order and **passes when the spec is run alone**, and `master`'s own `E2E (Playwright)` job is green, so it is order- and environment-specific to the local Windows stack. Left unfixed **by explicit decision** (2026-07-30): repairing it means editing a pre-existing test file, which is precisely what this feature's SC-007 constraint forbids, and it belongs to the upload slice, not here. **Three methodological corrections, all mine.** (1) A first attempt died before booting because the invocation piped PowerShell through `2>&1`: `docker compose` writes progress to **stderr**, and under the script's `$ErrorActionPreference = 'Stop'` the first progress line became a terminating error that jumped straight to the `finally` teardown — no probe ever ran. (2) I then started a second `e2e.ps1` while the first was still running. `scripts/e2e.ps1` hard-codes the compose project name `ladybug-e2e`, so the two collided: one run's `down -v` and `up --build` tore down and recreated the other's backend mid-test. **Two data points were corrupted this way and are void** — an isolated run's `upload:66` **cannot** be run concurrently with anything, and a third full run never reached Playwright at all (`backend did not become ready`, 180 s). (3) On that void evidence I had briefly concluded the two upload tests "trade failures"; the clean re-run shows they do not — it is the **same** test, `:66`, both clean times. Recorded because the wrong conclusion was the more interesting-sounding one, and it came entirely from an orchestration mistake rather than from anything in the codebase
- [ ] T067 **BLOCKED — needs a human and a real Google client; cannot be completed by the agent.** Walk quickstart.md §4.1 – §4.6 by hand against a real Google client on the dev stack — happy path, return visit, the US3 collision (including that the original password still works), cancellation and disabled, Back/Forward/Refresh at four points, and theme/responsive/keyboard. This is on the definition of done and is the only coverage the round trip has. **Why it is blocked, precisely**: `backend/.env` carries no `GOOGLE_*` keys at all (verified — not empty values, absent), so no client exists to drive; and even with one, the consent screen is a Google-hosted page requiring a real Google account to approve, which no automated path in this repo reaches. Research D16 made that deliberate — the round trip is the one thing the suites are designed *not* to simulate, which is exactly why this walkthrough is the only coverage it has and cannot be substituted with a green suite. **The feature is therefore not shippable on the strength of the other gates alone**: T064's 931 backend tests and the 7 green `google-signin` e2e cases prove every branch on our side of the boundary, and prove nothing about the half that runs at Google. To unblock: create the client per `docs/DEPLOYMENT.md` §3 "The Google OAuth client (017)", register `http://localhost:8000/api/auth/google/callback` byte for byte, put the id/secret in `backend/.env`, `docker compose restart backend` (dev opcache runs `validate_timestamps=0`), then walk quickstart §4.1–§4.6 — six scenarios, a few minutes
