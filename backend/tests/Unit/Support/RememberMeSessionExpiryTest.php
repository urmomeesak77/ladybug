...
/**
 * Direct proof of FR-004/SC-002 (Phase 4, US2) against the actual session handler Laravel
 * runs in every real environment (research D1: every env pins `SESSION_DRIVER=file`; only
 * phpunit.xml swaps in the `array` driver, for speed). ApplyRememberMeLifetime (T005) and
 * AuthController::login (T013) only ever change ONE input feeding this class:
 * config('session.lifetime'), which becomes config('remember.lifetime') for a remembered
 * session — fed into FileSessionHandler's constructor exactly as
 * SessionManager::createFileDriver() does. Exercising the real class here proves the
 * production mechanism directly.
 *
 * This intentionally does NOT drive the proof through simulated HTTP requests against the
 * `array` driver (as the rest of this feature's Feature tests do): that driver's Store is
 * cached as a container singleton for the lifetime of a whole test method (Laravel testing
 * quirk, already noted on AuthControllerTest::test_logout_succeeds_for_an_authenticated_user
 * for the closely related logout-revocation case), and Store::loadSession() merges a fresh
 * read onto the OLD in-memory attributes via array_replace() — so a second simulated
 * request's *failed* read is silently masked by data left over from the first. Talking to
 * FileSessionHandler directly (with a real temp directory and real file mtimes) has no such
 * cache to hide behind.
 *
